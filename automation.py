# automation.py
"""
This script stems from https://medium.com/swlh/automation-python-organizing-files-5d2b6b933402
"""
import os
...
# directory paths
user = os.getenv('USER')
#root_dir = 'D:\Python\Automation\Sandbox'.format(user)
py_dir = 'os.getcwd()'  #Intent: Python code lives here
root_dir = 'D:\Python\Automation\Sandbox' #Intent: Remote files to organize lives here
#root_dir = '/Users/{}/Downloads/'.format(user)
image_dir = 'D:\Python\Automation\Sandbox\images/'.format(user)
#image_dir = '/Users/{}/Downloads/images/'.format(user)
documents_dir = 'D:\Python\Automation\Sandbox\documents/'.format(user)
#documents_dir = '/Users/{}/Downloads/documents/'.format(user)
others_dir = 'D:\Python\Automation\Sandbox\others/'.format(user)
#others_dir = '/Users/{}/Downloads/others/'.format(user)
software_dir = 'D:\Python\Automation\Sandbox/softwares/'.format(user)
...
def get_non_hidden_files_except_current_file(root_dir):
  #return [f for f in os.listdir(root_dir) if os.path.isfile(f) and not f.startswith('.') and not f.__eq__(__file__) and not f.__contains__('automation')]
  return [f for f in os.listdir(root_dir) if os.path.isfile(f) and not f.startswith('.') and not f.__eq__(__file__) and not f.__contains__('automation')]
